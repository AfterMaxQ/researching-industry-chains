...
> 人如何在本地 Web 中高效查看来源证据、修正产业链草稿并完成最终审核，同时不绕开现有 CLI / Service / Runner 状态机。
...
3. 审核页必须清楚显示：**当前待审核来源属于哪个主题、来源是什么、真实证据是什么、Agent 草稿是什么、现在只需要人判断什么。**
4. 简单 review 可在 Queue 中快速处理，复杂 review 再进入 Full Review Workbench。
5. Full Review 采用 **Evidence-first** 心智：左侧原始来源证据，中间产业链草稿，右侧审核与决策。
6. 有产业链图时直接展示原图或必要的局部图，可点击放大；没有图时展示真正支持判断的正文 / 表格 / 交互证据。
7. **不使用正文摘要代替证据。** 人工审核需要来源原始证据，不需要 AI 再概括一遍文章。
8. 产业链树 + 企业归属是主要编辑模型，九字段 records 是底层业务协议，不是主要 UI。
9. 人工可修改、增加、删除节点和企业，也可修正企业挂载关系。
10. `draft_records=[]` 仍是合法 review，但 v1 不允许从空白开始纯人工构建完整来源。
11. `交回 AI 继续` 只作用当前 review_item / 当前 URL / 当前 reason，不建立域名规则。
12. Runner 页面能观察 Codex / Claude Code / Trae 等 Agent 主窗口当前 work，但 Web 不负责 Agent 调度。
13. Web 与 CLI 必须共用同一套 Python Service、RunnerStore、锁和状态机。
14. v1 保持轻量：无账号、无数据库、无 WebSocket、无消息队列。

最终体验目标：

> Agent 已经完成大部分研究与结构化工作；人打开审核页后，能直接对照真实证据，快速处理少量无法自动闭环的判断。
...
待审核卡必须优先展示：主题、来源、送审问题、待确认数量、是否已有草稿和下一步动作。
...
Quick Preview 只显示当前问题必要的上下文：

- 相关的产业链局部树；
- 相关来源证据入口；
- 当前 focus item。

若判断必须阅读大图、多个证据、完整树或改变结构，应升级到 Full Review。
...
处理一个 focus item 后自动切换下一个；全部处理后才突出“修正后通过”。
...
1. 左侧定位到该问题相关的图片 / 原文 / 表格证据；
2. 中间 Tree 定位到对应企业或节点；
3. 右侧只展示该问题的判断和动作。

如果证据是一段正文，可高亮目标企业 / 关键短语；如果证据是一张图，至少自动切换到对应图，不要求 v1 做像素级自动框选。

因此审核员不需要自己重新搜索“问题在哪、证据在哪、数据在哪”。
...
但当右侧当前正在展示“审核问题”时，Inspector 和 Review Decision 应保持同一区域内可切换，不额外增加第四栏。
...
- 同一 URL 在人工放行后遇到新问题，复用同一 review_item；
- 页面明确显示这是新问题；
- 旧 override 已消费；
- 当前 evidence / focus 只突出新问题。
...
image lightbox
shared tree editor
review inspector / decision panel
progress
completed
API client
working-copy state
...
4. **有产业链图就直接展示，并可放大查看。**
5. **没有图就展示真正支持判断的正文 / 表格 / 交互证据。**
6. **Evidence、Tree、Focus Item 三者必须联动。**
7. **产业链 Tree 是 UI；九字段 records 才是底层业务协议。**
8. **待审核数据不提前进入正式 source_groups / XLSX。**
9. **人工编辑使用 working copy，最终一次性提交。**
10. **review evidence 只保存当前人工审核真正需要的最小证据，不演变成 Evidence DB。**
11. **Agent 标准任务入口是 CLI；Human 标准审核入口是 Web。**
12. **CLI 与 Web 共用同一套 Python Service。**
13. **Web 不直接 PATCH 内部状态，也不直接修改 runner.json。**
14. **版本冲突必须显式失败，不能静默覆盖。**
15. **localhost v1 不为未来多人场景提前增加基础设施。**
16. **视觉语言统一采用暖色、克制、专业的 Warm Editorial Research Workbench。**
17. **最终目标是把人工工作压缩成少量有证据支撑的决策，而不是让人重新研究一遍来源。**
