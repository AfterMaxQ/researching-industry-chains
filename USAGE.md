...
第一次使用或只想跑一个主题时，按下面顺序执行。下面命令以 Windows PowerShell 为例。
...
$config = 'E:\data\topic_identity.yaml'
$runs = 'E:\researching-industry-chains\runs'
```

`$config`替换为实际的`topic_identity.yaml`路径。以后所有 CLI 命令都使用同一个`$runs`目录。

### 0.2 查询正式主题和 node_id

```powershell
industry-chain --runs-root $runs identity search --config $config --query '灵巧手丝杠'
```

从结果中确认正式主题、批准别名和主题名称。创建 Runner 后，使用 Runner 快照查询对应的`node_id`：

```powershell
$runner = (industry-chain --runs-root $runs runner create --name '灵巧手丝杠检索' --config $config | ConvertFrom-Json).data.runner_id
industry-chain --runs-root $runs topic search --runner-id $runner --query '灵巧手丝杠'
```

保存返回的`runner_id`和目标`node_id`。创建 Runner 后不要修改外部主题配置来影响本批次。

### 0.3 领取主题

```powershell
$node = '<node_id>'
$claim = industry-chain --runs-root $runs topic claim --runner-id $runner --node-id $node | ConvertFrom-Json
$claim.data.claim_token
$claim.data.lease_expires_at
```

把返回的`claim_token`和`lease_expires_at`保存下来。后续写入、修改、完成或失败都必须使用当前`claim_token`。

### 0.4 把启动提示词发给研究 Agent

把下面提示词中的占位符替换为领取结果，再发送给具备搜索、浏览器、截图和视觉能力的 Agent：

```text
请使用 researching-industry-chains Skill，先阅读 researching-industry-chains/SKILL.md。
...
只处理上述主题。先通过产业链证据门禁，再建立来源图表清单、实际查看产业链图和后续图表、按原图还原节点并核验企业直接归属。未通过证据门禁不得生成记录或写入来源组。图片或页面元素看不清时，裁切并放大后再判断。

每个来源完整解析后，通过 industry-chain CLI 标准输入一次性写入全部 records；不要创建中间 JSON、截图、PDF、脚本或日志文件。连续两轮无新增独立合格来源后提交主题终态。主题提交终态后立即汇报 node_id、主题、终态、来源组数、行数、来源 URL 和 XLSX 路径。
```

Agent 会负责搜索、读图、判断来源是否合格、写入来源组和提交主题终态。不要在 Agent 尚未完成来源扫描时手动写入数据。

### 0.5 查看进度和交付文件

```powershell
industry-chain --runs-root $runs runner status --runner-id $runner
```

交付文件位置：

```text
E:\researching-industry-chains\runs\<runner_id>\runner.json
E:\researching-industry-chains\runs\<runner_id>\<runner_id>_交付数据.xlsx
```

打开 XLSX 前先确认 Agent 已提交`completed`或`no_qualified_source`。如果状态为`in_progress`，不要打开或占用交付 XLSX，以免阻塞来源组写入。

### 0.6 三种常用运行方式

| 目的 | 操作 |
|---|---|
| 处理一个主题 | `topic search`找到`node_id`后，用`topic claim`指定领取，再派发一个 Agent |
| 处理全部待处理主题 | 创建 Runner 后，让 Agent 循环使用`topic claim-next`，直到没有`pending`主题 |
| 并行处理多个主题 | 先统计待处理`node_id`数量，再均分范围；每个 Agent 的提示词明确自己的`node_id`范围 |

失败主题补跑：

```powershell
industry-chain --runs-root $runs topic claim --runner-id $runner --node-id <failed_node_id> --reopen
```
