...
发现产业链图、产业链结构表或其它能够可靠表达产业链结构的视觉内容时，立即优先处理，不等待正文解析完成。

处理顺序：

1. 产业链图或其它结构视觉证据；
2. 图中直接展示的节点企业；
3. 企业补充图、企业分布图、企业表格和企业列表；
4. 正文中的明确产业链结构和企业归属。

视觉内容确认的 Tree 作为当前来源持续维护的结构骨架。后续正文主要用于补充，不得仅因正文信息量更大而覆盖、改名、合并、重排或替换已经由视觉证据确认的节点结构。

企业信息不采用“只取最高优先级证据”的方式。同一来源中，只要产业链图、企业补充图、表格、企业列表或正文能够直接证明企业归属于某个节点，就将该企业增量合并到当前内存 Tree 对应节点的 `companies`。后续证据只能追加新企业、对同一节点重复企业去重，或在明确证据冲突且无法消解时进入 `review`；不得重新生成 `companies` 并导致已经确认的企业消失。

这里的增量合并仅发生在当前来源尚未提交的内存 Tree 中。完整来源扫描和覆盖检查结束后，仍然只通过一次 `source submit` 提交完整 SourceResult。
...
企业清单必须覆盖整个来源中的产业链图、补充节点图、企业图表、企业列表和正文，而不是只覆盖最终用于还原 Tree 的主要证据。每个可读企业都必须已挂载，或已明确判断为无法归属；图中有而正文没有、正文有而图中没有的企业，只要存在直接节点归属证据都必须保留。无法归属的企业不要伪造节点归属；可在最终 `description` 中写 `发现但无法归属：A公司、B公司`。

同一企业在同一节点重复出现时只保留一次；来源明确将同一企业归属于多个不同节点时可以分别保留。视觉证据和正文证据对同一企业的节点归属发生明确冲突且无法消解时，不选择其中一方覆盖另一方，提交 `review`。
...
→ 产业链图优先建立当前内存 Tree
→ 图中企业立即挂载
→ 企业补充图、企业表格和企业列表继续增量合并
→ 视觉内容处理完成后读取正文并补充节点和企业
→ 建立完整节点清单和企业清单
→ 节点覆盖检查 + 企业覆盖检查
→ 形成完整 SourceResult
→ source submit
...
企业覆盖检查必须重新核对同一来源中的产业链图、补充节点图、企业图表、企业列表和正文。每个可读企业都必须已经挂载到直接证据支持的节点，或明确判断为无法归属；不得因为企业只存在于图片中而遗漏。

用内存维护来源、Tree、节点和企业清单，不写入 Runner，不边读边调用 CLI 增量持久化。不要创建中间 JSON；通过标准输入一次提交完整 SourceResult：
